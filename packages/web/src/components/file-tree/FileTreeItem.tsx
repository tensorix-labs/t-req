...
import { ChevronIcon, FileIcon, FolderIcon, HttpFileIcon, ScriptFileIcon } from '../icons';
...
export function FileTreeItem(props: FileTreeItemProps) {
  ...
  const isExpanded = () => props.flatNode.isExpanded;
  ...
  const itemClasses = () => {
    const base =
      'flex items-center gap-1 py-1.5 pr-3 cursor-pointer rounded-treq transition-all duration-150 outline-none';
    const hover = 'hover:text-treq-accent dark:hover:text-treq-accent';
    const focus =
      'focus-visible:outline-2 focus-visible:outline-treq-accent focus-visible:-outline-offset-2';
  ...
  const chevronClasses = () => {
    ...
    const color = props.isSelected
      ? 'text-treq-accent'
      : 'text-treq-text-muted dark:text-treq-dark-text-muted';
  ...
  const iconClasses = () => {
    ...
    if (node().isDir && isExpanded())
      return 'flex items-center justify-center w-4 h-4 mr-1 text-treq-accent';
    if (node().isDir)
  ...
  return (
    ...
    >
      ...
      <span class="flex-1 text-sm whitespace-nowrap overflow-hidden text-ellipsis min-w-0">
        {node().name}
      </span>
    </div>
  );
