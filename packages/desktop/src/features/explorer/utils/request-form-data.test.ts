...
describe('areFormDataFieldsEqual', () => {
  it('returns true when fields and order match', () => {
    expect(
      areFormDataFieldsEqual(
        [
          { name: 'title', value: 'hello', isFile: false },
          { name: 'document', value: '', isFile: true, path: './payload.json', filename: '' }
        ],
        [
          { name: 'title', value: 'hello', isFile: false },
...
describe('serializeFormDataBody', () => {
  it('serializes text and file fields with parser-compatible syntax', () => {
    expect(
      serializeFormDataBody([
  ...
  it('returns an empty string when no rows are serializable', () => {
    expect(serializeFormDataBody([{ name: '', value: '', isFile: false }])).toBe('');
  });

  it('normalizes file paths to parser-compatible syntax', () => {
    expect(
      serializeFormDataBody([
